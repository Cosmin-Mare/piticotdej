...
export default function AdminEchipaPage() {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      void logActivity(user.email, "A adăugat un membru nou (echipa)");
      router.push(`/admin/echipa/${id}`);
    } catch {
  ...
  return (
    <AdminShell title="Echipa" backHref="/admin">
      <p style={{ color: "var(--ink-soft)", marginTop: 0, marginBottom: 8 }}>
        Educatoare și personal didactic — apar pe pagina Echipa. Folosește săgețile pentru ordine și „Ascunde” în loc de ștergere.
      </p>
      <SecondaryPageLink href="/admin/continut/echipa">
        Editează textele paginii Echipa →
      </SecondaryPageLink>
              ...
              <ul className="admin-anunturi-list">
                {items.map((item, i) => (
                  <li key={item.id} className="admin-list-row">
                    ...
                    />
                    <Link href={`/admin/echipa/${item.id}`} className="admin-anunturi-row" style={{ flex: 1, border: 0 }}>
                      <div>
